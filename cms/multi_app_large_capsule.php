<?php
/**
 * Multi-App Large Capsule (2006+ themes)
 * PHP adaptation of the 2007 capsule_lg.php that loads app data from store_apps table
 * and multi-app configurations from multicapsule table
 * PHP >= 7.4
 */
...
// Get capsule group ID from URL parameter
$group = $_GET['group'] ?? '';
if (empty($group)) {
...
// Load multi-app data from database
$stmt = $db->prepare('
    SELECT m.appid, m.image_path, m.price, a.name 
    FROM multicapsule m 
    LEFT JOIN store_apps a ON m.appid = a.appid 
    WHERE m.`group` = ? 
...
foreach ($rows as $row) {
    $items[] = [
        'img'   => $row['image_path'] ? '../storefront/images/capsules/' . $row['image_path'] : '',
        'title' => $row['name'] ?: 'Unknown Game',
        'price' => fmt($row['price'], 'currency'),
        'url'   => $row['appid'] ? "index.php?area=game&AppId={$row['appid']}" : '#'
    ];
}
...
<title>Multi-App Large Capsule</title>
<style>
/* ---------- universal ---------- */
...
<div id="capsule">
    <img id="capImg" src="" alt="Game Screenshot">
...
function show(i){
    ...
    img.onclick = () => window.open(o.url, '_parent');
