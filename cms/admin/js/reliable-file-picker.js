...
class ReliableFilePicker {
    constructor() {
        ...
        this.fileCache = {}; // Cache for file listings
    ...
    init() {
        ...
        // Set up global drag/drop prevention after modal is created
    ...
    setupGlobalDragDropPrevention() {
        try {
            ...
            console.log('ReliableFilePicker: Global drag/drop prevention setup complete');
        } catch (error) {
            console.error('ReliableFilePicker: Error setting up drag/drop prevention:', error);
        }
    }
    ...
    async loadFiles() {
        ...
        if (this.fileCache[this.currentUploadPath]) {
            console.log('ReliableFilePicker: Loading from cache:', this.currentUploadPath);
            this.browsedFiles = this.fileCache[this.currentUploadPath];
        ...
        try {
            ...
            if (data.success && data.files) {
                ...
                this.fileCache[this.currentUploadPath] = data.files;
                    ...
                    onload: (response) => {
                        try {
                            ...
                            if (data.success) {
                                ...
                                // Clear cache and reload files to show new upload
                                delete this.fileCache[this.currentUploadPath];
                                this.loadFiles();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        // Preload files for faster modal opening
        setTimeout(() => {
            preloadFilePickerData();
        }, 1000);
    } catch (error) {
...
// Preload file picker data for better performance
function preloadFilePickerData() {
    try {
        console.log('ReliableFilePicker: Preloading file data...');
        
        // Preload common image directories
        ...
        commonPaths.forEach(path => {
            ...
            .then(text => {
                try {
                    const data = JSON.parse(text);
                    if (data.success && data.files) {
                        console.log(`ReliableFilePicker: Preloaded ${data.files.length} files from ${path}`);
                    }
                } catch (e) {
                    console.log(`ReliableFilePicker: Preload failed for ${path}:`, e.message);
                }
            })
            .catch(error => {
                console.log(`ReliableFilePicker: Preload network error for ${path}:`, error);
            });
        });
    } catch (error) {
        console.log('ReliableFilePicker: Preload error:', error);
    }
}
...
function initReliableFilePickerButtons() {
    try {
        ...
        if (buttons.length === 0) {
            console.log('ReliableFilePicker: No buttons found with [data-reliable-file-picker] attribute');
            return;
        }
        
        buttons.forEach((button, index) => {
            console.log(`ReliableFilePicker: Setting up button ${index + 1}:`, button);
            
            button.addEventListener('click', (e) => {
                console.log('ReliableFilePicker: Button clicked!', e.target);
                e.preventDefault();
                ...
                console.log('ReliableFilePicker: Button config:', {
                    uploadPath,
                    targetInput,
                    previewElement
                });
                ...
                openReliableFilePicker(uploadPath, (selectedPath) => {
                    ...
                    if (targetInput) {
                        ...
                        if (input) {
                            ...
                            console.log('Updated input:', targetInput, 'with value:', selectedPath);
                        } else {
                    ...
                    if (previewElement) {
                        ...
                        if (preview) {
                            ...
                            console.log('Updated preview:', previewElement, 'with src:', selectedPath);
                        } else {
                            console.warn('Preview element not found:', previewElement);
                        }
                    }
                });
            });
        ...
        console.log('ReliableFilePicker: Button initialization complete');
    } catch (error) {
        console.error('ReliableFilePicker: Error initializing buttons:', error);
    }
}