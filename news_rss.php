...
$db = cms_get_db();
$items = $db->query("SELECT id,title,content,publish_date FROM news WHERE publish_at<=NOW() AND status='published' ORDER BY publish_at DESC LIMIT 20")->fetchAll(PDO::FETCH_ASSOC);
