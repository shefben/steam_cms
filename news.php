...
$page_title = cms_get_setting('steam_news_page_title', 'Steam News');
$db = cms_get_db();
$theme = cms_get_setting('theme', '2004');
$sidebar_themes = ['2006_v1', '2006_v2', '2007_v1', '2007_v2'];

if (in_array($theme, $sidebar_themes, true)) {
    $content = '';
    if (isset($_GET['news']) || isset($_GET['id'])) {
        ...
        if ($row) {
            ...
            $content .= "<h1 style='line-height: 15px;'><a href='$self_link' style='text-decoration: none;'>$title</a></h1>";
            $content .= "<h2>$date - $author</h2><hr>";
            $content .= $row['content'];
            $content .= '<p><a href="index.php?area=news" style="text-decoration: none;"><i>return to news page</i></a></p>';
        } else {
