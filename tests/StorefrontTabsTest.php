...
$pdo->exec('CREATE TABLE storefront_tab_games (tab_id INT, appid INT, ord INT)');
$pdo->prepare('INSERT INTO storefront_tabs(theme,title,ord) VALUES(?,?,?)')->execute(["2007_v2","Test",1]);
$id = $pdo->lastInsertId();
$pdo->prepare('INSERT INTO storefront_tab_games(tab_id,appid,ord) VALUES(?,?,?)')->execute([$id,100,1]);
