...
function OnMovieComplete() 
{
	if ( g_player )
	{
		g_player.OnMovieComplete( g_player.m_activeItem );
		//setTimeout( g_player.OnMovieComplete.bind( g_player, g_player.m_activeItem ), 1500 );
	}
}
...
var HighlightPlayer = Class.create( {
	...
	LoadMovie: function( id )
	{
		...
		{
			swfobject.embedSWF( "http://cdn.store.steampowered.com/public/swf/videoPlayer.swf?v=8", strTarget, rgFlashVars['STAGE_WIDTH'], rgFlashVars['STAGE_HEIGHT'], "9", false, rgFlashVars, {wmode: "opaque", allowScriptAccess: "always", allowFullScreen: "true" } );
			if ( $(strTarget) && $(strTarget).tagName == 'DIV' )
			{
