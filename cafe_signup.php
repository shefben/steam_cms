...
if($_SERVER['REQUEST_METHOD']=='POST'){
    ...
    if(!preg_match('/^[0-9\-]+$/',$data['phone'])) $errors[]='Phone must be numeric';
    if($data['fax'] && !preg_match('/^[0-9\-]+$/',$data['fax'])) $errors[]='Fax must be numeric';
    if(!preg_match('/^[0-9]+$/',$data['zip'])) $errors[]='Invalid zip code';
    if($data['ship_zip'] && !preg_match('/^[0-9]+$/',$data['ship_zip'])) $errors[]='Invalid shipping zip';
    if(!filter_var($data['email'],FILTER_VALIDATE_EMAIL)) $errors[]='Invalid email';
    if($data['contact_email'] && !filter_var($data['contact_email'],FILTER_VALIDATE_EMAIL)) $errors[]='Invalid contact email';
    if(!$errors){
...
?>
<div class="content" id="container" style="background:#3e4637;">
  <h1>NEW CUSTOMER / CYBER CAF&Eacute; PROGRAM SIGN-UP</h1>
  <h2>GIVE <em>YOUR CUSTOMERS THE GAMES THEY WANT!</em></h2><img src="img/Graphic_Box.jpg" height="6" width="24" alt=""><br>
  <br>
  <div class="narrower">
    ...
    <p>&nbsp;</p>
    <?php if(!empty($errors)): ?><ul style="color:red;"><?php foreach($errors as $e) echo '<li>'.htmlspecialchars($e).'</li>'; ?></ul><?php endif; ?>
    <?php if(isset($success)): ?>
        <p>Thank you for registering!</p>
    <?php else: ?>
    <form method="post">
      <table cellspacing="6" width="100%" style="background:#3e4637;">
